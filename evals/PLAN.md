...
- prose rule requiring a flag on every invocation of a formatter ("always pass --dirty") with no native config to enforce it → expect an agent-harness PreToolUse hook, then the prose trimmed to the positive form
